...
from focuspool import WEIGHTS_DIR

# Task descriptions (MimicGen)

MIMICGEN_DESCRIPTIONS = {
    "coffee": "Make coffee using the coffee machine",
...
_TASK_EMB_CACHE_PATH = WEIGHTS_DIR / "task_emb_cache.npz"
_TASK_CACHE_LOCK = threading.RLock()
_LOCK_POLL_SECONDS = 0.1


def _default_cache_path() -> Path:
    return _TASK_EMB_CACHE_PATH
...
def env_name_to_instruction(env_name: str) -> str:
    ...
    for key, desc in sorted(MIMICGEN_DESCRIPTIONS.items(), key=lambda item: -len(item[0])):
        if key in norm:
            return desc.lower()
...
def _cache_covers_current_tasks(texts: Sequence[str]) -> bool:
    cached = {str(text).strip().lower() for text in texts}
    expected = {str(text).strip().lower() for text in MIMICGEN_DESCRIPTIONS.values()}
...
def setup_task_embedding_cache(cache_path: Optional[str] = None) -> None:
    ...
    - Else: build it (encode all task descriptions), save it atomically, then load it.
    """
    ...
    with _TASK_CACHE_LOCK:
        ...
        with _TaskEmbeddingCacheFileLock(p):
            ...
            if cache is not None and not _cache_covers_current_tasks(cache[0]):
                cache = None
...
def _text_to_task_embedding(text: Union[str, Sequence[str]]) -> torch.Tensor:
    ...
    if missing:
        ...
        for i in missing:
            out[i] = encode_text_clip(keys[i], device=device)[0]
...
def env_name_to_robot(env_name: str) -> str:
    ...
    """
    norm = _normalize_env_name(env_name)
    for key, robot in sorted(ROBOT_ASSIGMNENT.items(), key=lambda item: -len(item[0])):
        if key in norm:
            return robot.lower()
    raise ValueError(f"Unknown env name '{env_name}' (normalized '{norm}')")
