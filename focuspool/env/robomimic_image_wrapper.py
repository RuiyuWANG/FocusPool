...
class RobomimicImageWrapper(gym.Env):
    def __init__(
        ...
        if self.enable_oracle_subtask_info:
    ...
    def step(self, action):
        ...
        if self.enable_oracle_subtask_info:
            info = dict(info)
            info.update(self.get_oracle_subtask_info())
        return obs, reward, done, info

    def get_oracle_subtask_info(self):
        """Return live MimicGen oracle subtask object-ref and projection info."""
    ...
    def _oracle_projection_size(self):
        ...
        if h is None or w is None:
            shape = self.shape_meta["obs"][f"{self.oracle_projection_camera}_image"][
                "shape"
            ]
            h = int(shape[-2])
            w = int(shape[-1])
    ...
    def _draw_prediction_overlay(self, img: np.ndarray) -> np.ndarray:
        ...
        for item in self.prediction_video_boxes:
            ...
            if "points_px" in item:
                ...
                if "point_radius_px" in item:
                    point_radius = float(item["point_radius_px"])
                    radius_scale = 0.5 * (x_scale + y_scale)
                    radius = max(1, int(round(point_radius * radius_scale)))
