...
from focuspool.env.robomimic_image_wrapper import RobomimicImageWrapper
from focuspool.env_runner.async_vector_env import AsyncVectorEnv
from focuspool.env_runner.multistep_wrapper import MultiStepWrapper
from focuspool.env_runner.video_recording_wrapper import (
    VideoRecorder,
    VideoRecordingWrapper,
)
from focuspool.env.mjcf_texture import list_texture_files
...
from focuspool.dataset.cache import load_metadata, resolve_cache_dir
from focuspool import TEXTURES_DIR


install_table_texture_hook()

ROLLOUT_CAMERA_RESOLUTION = 84
...
def default_rollout_shape_meta(default_res: int = ROLLOUT_CAMERA_RESOLUTION) -> dict:
    return {
        "obs": {
            "agentview_image": {
...
def _is_retriable_vector_env_init_error(exc: Exception) -> bool:
    if isinstance(exc, (BrokenPipeError, EOFError)):
        return True
    if isinstance(exc, OSError) and getattr(exc, "errno", None) in {32, 104}:
        return True
    message = str(exc).lower()
    return ("broken pipe" in message) or ("egl_bad_alloc" in message)


def _build_vector_env_with_backoff(
    *,
    env_meta: dict,
    default_shape_meta: dict,
    n_envs: int,
    ...
    current_n_envs = max(int(n_envs), 1)
    while True:
        try:
            return _build_vector_env(
                env_meta=env_meta,
            ...
            if current_n_envs <= 1 or not _is_retriable_vector_env_init_error(exc):
                raise
            next_n_envs = max(1, current_n_envs // 2)
            if next_n_envs == current_n_envs:
                next_n_envs = current_n_envs - 1
            print(
                "[robomimic_setup] Vector env init failed with "
                f"n_envs={current_n_envs} ({type(exc).__name__}: {exc}). "
                f"Retrying with n_envs={next_n_envs}."
            )
            current_n_envs = next_n_envs
...
def _load_env_meta_from_cache(
    *,
    ...
    def _resolve_source_hdf5_path(dataset_path: str, cache_path: Path, meta: dict) -> Path:
        source_hdf5 = meta.get("source_hdf5")
        ...
        if dataset_file.is_file() and dataset_file.exists():
            return dataset_file

        candidates = []
        for root in {dataset_file, cache_path.parent}:
            if not root.exists() or not root.is_dir():
                continue
            exact = root / f"{root.name}.hdf5"
            candidates.extend([exact, *sorted(root.glob("*.hdf5"))])
        ...
        preferred = dataset_file.parent / f"{dataset_file.stem}.hdf5"
        if len(deduped) > 1 and preferred.exists():
            return preferred.resolve()

        raise FileNotFoundError(
            "Could not infer source HDF5 for cache missing env metadata. "
            f"Checked dataset_path={dataset_path!r} cache_dir={str(cache_path)!r} "
            f"candidates={list(map(str, deduped))}"
        )
    ...
    if not available:
        source_hdf5 = _resolve_source_hdf5_path(dataset_path, cache_path, meta)
        env_meta = FileUtils.get_env_metadata_from_dataset(str(source_hdf5))
        if env_name is not None:
            env_meta["env_name"] = str(env_name)
        return copy.deepcopy(env_meta)
...
def build_robomimic_runner_setup(
    *,
    ...
    default_res = ROLLOUT_CAMERA_RESOLUTION
    ...
    env, env_fns = _build_vector_env_with_backoff(
        env_meta=env_meta,
        default_shape_meta=default_shape_meta,
        n_envs=n_envs,
        render_obs_key=render_obs_key,
