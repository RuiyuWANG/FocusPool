...
from focuspool.focus_toolbox.viz import visual_focus_records_to_video_boxes
...
def extract_policy_prediction_video_boxes(
    ...
    *,
    ...
    last_video_items = []
    records = getattr(obs_encoder, "last_visual_focus_records", None)
    if records:
        last_video_items.extend(visual_focus_records_to_video_boxes(records))
    encoder_items = getattr(obs_encoder, "last_video_boxes", None)
    if encoder_items:
        for item in encoder_items:
            if records and isinstance(item, dict) and "box_px" in item:
                continue
        ...
        for env_idx in range(count):
            ...
            if points_arr is not None:
                ...
                if "point_radius_px" in item:
                    overlay["point_radius_px"] = float(item["point_radius_px"])
