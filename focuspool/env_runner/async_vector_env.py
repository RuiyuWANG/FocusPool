...
def _default_mp_context():
    """Pick a robust multiprocessing context for environment workers.

    Prefer ``spawn`` for EGL rendering, since raw EGL contexts are often not
    fork-safe. Otherwise prefer ``fork`` on POSIX so CLI, REPL, and IDE
    launches do not depend on ``spawn`` re-importing the main module from a
    real file path. Set ``FOCUSPOOL_MP_CONTEXT`` to override explicitly when
    needed.
    """
    override = os.getenv("FOCUSPOOL_MP_CONTEXT")
    if override:
        return override
    if os.getenv("MUJOCO_GL", "").lower().strip() == "egl":
        return "spawn"
    if os.name == "posix":
        return "fork"
    return "spawn"
