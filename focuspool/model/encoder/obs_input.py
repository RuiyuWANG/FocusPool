...
consumed by policy encoder variants.
"""
...
from focuspool.util.formatting import find_all_keys
...
class ObsInputProcessor(nn.Module):
    ...
    - build validated `composer_in` tensors used by pooling/RVT2 components.
    """
    ...
    def obs_to_input(
        self, obs: dict, normalizer, resize: bool = False
    ) -> EncoderInputs:
        ...
        agentview_image = normalize_imagenet(obs_flat[k["agentview"]])
        if resize:
            agentview_image = resize_image(agentview_image, self.input_res)

        if self.enable_eih:
            eih_image = normalize_imagenet(obs_flat[k["in_hand"]])
            if resize:
        ...
        gripper_open = (
            torch.abs(obs_norm[k["gripper"]][:, 0] - obs_norm[k["gripper"]][:, 1]) - 1.0
        )

        composer_in = {
            "eef_pos": obs_norm[k["eef_pos"]],
            "eef_rot": obs_norm[k["eef_rot"]][..., :6],
            "gripper": obs_norm[k["gripper"]],
        ...
        proprio = torch.cat(
            [composer_in[n] for n in ["eef_pos", "eef_rot", "gripper"]], dim=-1
        )
