"""Policy observation encoder using an optional RVT2Heatmap crop model."""
...
from focuspool.model.encoder.focus_view_transform import FocusViewTransform
from focuspool.model.common.resnet import build_resnet18_backbone
from focuspool.focus_toolbox.prediction import VisualFocusPrediction, VisualFocusRecord
from focuspool.model.encoder.obs_input import EncoderInputs, ObsInputProcessor
from focuspool.util.visualization import visualize
from focuspool.focus_toolbox.geometry import normalize_box


class FocusConditionedObsEncoder(ObsInputProcessor):
    """Encode observations with RVT2-guided crops and ResNet backbones."""
    ...
    def __init__(
        ...
        *,
        ...
        # Keep preprocessing aligned with the focus transform input resolution.
    ...
    def forward(self, obs, obs_index=None):
        ...
        processed_views = self.focus_view_transform(
            views, composer_in=enc_in.composer_in, obs_index=obs_index
        )
    ...
    def set_normalizer(self, normalizer) -> None:
        """Set observation normalizer used by the focus view transform."""
        self.focus_view_transform.set_normalizer(normalizer)

    def encode_agentview(self, image, box_px):
        """Encode agentview image and optionally FiLM-condition on RVT2 box."""
    ...
    def process_obs(self, obs: Dict[str, torch.Tensor]) -> EncoderInputs:
        ...
        normalizer = self.focus_view_transform.normalizer
        return self.obs_to_input(obs, normalizer, resize=True)
