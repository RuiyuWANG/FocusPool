"""Rendering and plotting helpers for visual focus."""
...
from focuspool.focus_toolbox.prediction import VisualFocusRecord


def visual_focus_records_to_video_boxes(
    records: list[VisualFocusRecord],
) -> list[dict]:
    """Convert focus records to the rollout overlay box format."""
    out = []
    for record in records:
        box_px = record.prediction.box_px
        if box_px is None:
            continue
        out.append(
            {
...
def overlay_heatmap(
    ...
    base = image.detach().cpu().clamp(0, 1)
...
def draw_patch_debug(
    *,
    ...
    if h != w:
        image = resize_image(image[None], max(h, w))[0]
        h = w = max(h, w)
    ...
    label = f"src {source_frame} -> key {target_frame} | pred {pred_patch} gt {target_patch}"
