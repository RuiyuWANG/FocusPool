...
from focuspool.workspace.training_io import JsonLogger, TopKCheckpointManager
from focuspool.model.common.lr_scheduler import get_scheduler
from focuspool.model.diffusion.ema_model import EMAModel
from focuspool.workspace.base_workspace import BaseWorkspace
from omegaconf import OmegaConf
from torch.utils.data import DataLoader
from einops import rearrange

from focuspool.policy.diffusion_policy import DiffusionPolicy
from focuspool.util.formatting import fold_path_from_marker, pretty_print_nested
...
class TrainFocusPolicyWorkspace(BaseWorkspace):
    ...
    def run(self):
        ...
        if tags is not None:
            logging_cfg["tags"] = [tag for tag in tags if tag]
        ...
        with JsonLogger(log_path) as json_logger:
            while self.epoch < cfg.training.num_epochs:
                if hasattr(self.model.obs_encoder, "set_epoch"):
                    self.model.obs_encoder.set_epoch(self.epoch)
                if self.ema_model is not None and hasattr(
                    self.ema_model.obs_encoder, "set_epoch"
                ):
                    self.ema_model.obs_encoder.set_epoch(self.epoch)
                step_log = dict()
                ...
                with tqdm.tqdm(
                    ...
                    for batch_idx, batch in enumerate(tepoch):
                        ...
                        if hasattr(self.model, "get_last_attention_prior_metrics"):
                            step_log.update(
                                self.model.get_last_attention_prior_metrics()
                            )
