"""Train the optional RVT2Heatmap baseline on heuristic keypoint labels."""
...
from focuspool.dataset.rvt2_heatmap_dataset import (
...
from focuspool.dataset.sampler import build_task_balanced_sampler
from focuspool.dataset.mimicgen_dataset import MimicGenDataset
from focuspool.model.rvt2_heatmap import (
    PatchFeatureBackbone,
    PatchActivationHead,
)
from focuspool.util.image_ops import normalize_imagenet, resize_image
from focuspool.focus_toolbox.viz import draw_patch_debug
from focuspool.focus_toolbox.config import load_rvt2_heatmap_config
from focuspool.util.task_meta import NUM_ROBOTS
from focuspool.workspace.base_workspace import BaseWorkspace
...
def _save_visualization_grid(
    *,
    ...
    with torch.no_grad():
        ...
        image = resize_image(image, dino_image_size)
        image_norm = normalize_imagenet(image)
...
def _prepare_model_state(runtime: dict) -> dict:
    ...
    head_config = {
        ...
        "task_emb_dim": int(rvt2_heatmap_cfg["task_emb_dim"]),
        "num_robots": int(NUM_ROBOTS),
        "query_hidden_mult": int(rvt2_heatmap_cfg["query_hidden_mult"]),
        "proprio_mode": str(rvt2_heatmap_cfg["proprio_mode"]),
        "proprio_dim": int(rvt2_heatmap_cfg["proprio_dim"]),
        "language_seq_len": int(rvt2_heatmap_cfg["language_seq_len"]),
    ...
    for epoch in range(start_epoch, int(args.epochs) + 1):
        ...
        if wandb_run is not None:
            ...
            if "visualization" in payload:
                step_log["visualization"] = wandb.Image(payload["visualization"])
