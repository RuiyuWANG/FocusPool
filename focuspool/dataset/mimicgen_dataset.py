...
from focuspool.model.common.normalizer import (
    LinearNormalizer,
...
from focuspool.dataset.action import load_action_array, validate_action_rep
from focuspool.dataset.cache import (
    check_cache,
...
from focuspool.dataset.oracle_cache import load_oracle_info
from focuspool.util.image_ops import decode_jpg_bytes
from focuspool.util.formatting import is_matched_key


def _normalize_task_embedding_array(arr: np.ndarray, *, name: str) -> np.ndarray:
    """Accept old caches with [N, 1, D] embeddings and normalize to [N, D]."""
    arr = np.asarray(arr, dtype=np.float32)
    while arr.ndim > 2 and arr.shape[1] == 1:
        arr = np.squeeze(arr, axis=1)
    if arr.ndim != 2:
        raise ValueError(f"{name} must be [N, D], got {arr.shape}")
    return arr.astype(np.float32, copy=False)


def _normalize_task_language_tokens_array(arr: np.ndarray, *, name: str) -> np.ndarray:
    """Accept old caches with [N, 1, L, D] language tokens and normalize to [N, L, D]."""
    arr = np.asarray(arr, dtype=np.float32)
    while arr.ndim > 3 and arr.shape[1] == 1:
        arr = np.squeeze(arr, axis=1)
    if arr.ndim != 3:
        raise ValueError(f"{name} must be [N, L, D], got {arr.shape}")
    return arr.astype(np.float32, copy=False)
...
class MimicGenDataset(torch.utils.data.Dataset):
    """Torch dataset for policy training/evaluation on cached MimicGen trajectories."""
    ...
    def __init__(
        ...
        self.rvt2_attention_target_cfg = (
            dict(rvt2_attention_target) if rvt2_attention_target is not None else {}
        )
        self.rvt2_attention_target_enabled = _strict_bool(
            self.rvt2_attention_target_cfg.get("enabled", False),
            "rvt2_attention_target.enabled",
        )
        if self.rvt2_attention_target_enabled and not self.include_oracle_info:
            raise ValueError(
                "RVT2 attention targets require include_oracle_info=True."
            )
        ...
        self.cache_dir = (
            ...
            else str(default_cache_dir_for_dataset(dataset_path))
        )
    ...
    def set_active_demos(self, n_demo=None):
        ...
        self._build_rvt2_attention_targets()
    ...
    def get_normalizer(self) -> MultiRobotLinearNormalizer:
        ...
        for robot_id in np.unique(step_robot_ids).tolist():
            ...
            for key in self.lowdim_keys:
                ...
                if is_matched_key("pos", key) or is_matched_key("qpos", key):
    ...
    def _make_sample(self, eps_index, obs, action, obs_indices):
        ...
        if self.oracle_info is not None and not self.rvt2_attention_target_enabled:
            sample["oracle_info"] = self._get_oracle_info(obs_indices)
        if self.rvt2_attention_targets is not None:
            sample["rvt2_attention_target"] = self._get_rvt2_attention_target(
                obs_indices
            )
        return sample

    def _get_oracle_info(self, obs_indices):
        """Fetch optional per-frame oracle labels aligned with active frame indices."""
        active_obs_indices = np.asarray(obs_indices, dtype=np.int64)
        global_obs_indices = self.active_to_global_indices(active_obs_indices)
    ...
    def _slice_oracle_info(self, value, indices):
        if isinstance(value, dict):
            return {k: self._slice_oracle_info(v, indices) for k, v in value.items()}
        out = value[indices]
        if out.dtype.kind in ("U", "S"):
            return out
        if out.dtype == np.uint8 or out.dtype == np.bool_:
            return out.astype(np.uint8)
        if out.dtype.kind in ("i", "u"):
            return out.astype(np.int64)
        return out.astype(np.float32)

    def _get_rvt2_attention_target(self, obs_indices):
        active_obs_indices = np.asarray(obs_indices, dtype=np.int64)
        targets = self.rvt2_attention_targets
        return {
            view: {
                "xy_px": target["xy_px"][active_obs_indices].astype(
                    np.float32, copy=False
                ),
                "valid": target["valid"][active_obs_indices].astype(
                    np.bool_, copy=False
                ),
                "source_size": np.asarray(target["source_size"], dtype=np.int64),
            }
            for view, target in targets.items()
        }

    def _build_rvt2_attention_targets(self) -> None:
        if not self.rvt2_attention_target_enabled:
            self.rvt2_attention_targets = None
            return

        from focuspool.focus_toolbox.rvt2_heatmap import (
            _project_world_xyz_to_row_col,
            discover_rvt2_heatmap_keypoints,
            gripper_open_from_signal,
        )

        if self.oracle_info is None:
            raise ValueError("RVT2 attention targets require oracle info.")

        cfg = self.rvt2_attention_target_cfg
        configured_views = cfg.get("views")
        if configured_views is None:
            view_cfgs = {
                "agentview": {
                    "camera": str(cfg.get("camera", "agentview")),
                }
            }
        else:
            view_cfgs = {}
            for view, view_cfg in dict(configured_views).items():
                view = str(view)
                view_cfg = dict(view_cfg or {})
                view_cfgs[view] = {
                    "camera": str(view_cfg.get("camera", view)),
                }
            if not view_cfgs:
                raise ValueError("rvt2_attention_target.views must not be empty")

        camera_matrices_by_view = {}
        for view, view_cfg in view_cfgs.items():
            camera = str(view_cfg["camera"])
            camera_key = f"camera_matrix_{camera}"
            if camera_key not in self.oracle_info:
                available = ", ".join(sorted(self.oracle_info.keys()))
                raise ValueError(
                    f"RVT2 attention targets for view {view!r} require "
                    f"oracle/{camera_key}.npy. Available oracle keys: {available}"
                )
            camera_matrices_by_view[view] = np.asarray(
                self.oracle_info[camera_key], dtype=np.float32
            )
        for key in ("robot0_eef_pos", "robot0_gripper_qpos"):
            if key not in self.lowdim:
                raise ValueError(f"RVT2 attention targets require lowdim/{key}.npy.")

        source_size = int(self.meta.get("image_size") or self.image_size or 0)
        if source_size <= 0:
            raise ValueError(
                "RVT2 attention targets require a positive cache image size."
            )

        targets = {
            view: {
                "xy_px": np.full(
                    (self.n_samples_active, 2), np.nan, dtype=np.float32
                ),
                "valid": np.zeros((self.n_samples_active,), dtype=np.bool_),
                "source_size": source_size,
            }
            for view in view_cfgs
        }

        for episode_idx in range(self.n_demo_active):
            active_indices = self.episode_active_indices(episode_idx)
            global_indices = self.active_to_global_indices(active_indices)
            eef_pos = self.lowdim["robot0_eef_pos"][global_indices].astype(
                np.float32,
                copy=False,
            )
            gripper_qpos = self.lowdim["robot0_gripper_qpos"][global_indices].astype(
                np.float32,
                copy=False,
            )
            joint_velocities, _ = self.optional_episode_lowdim(
                episode_idx,
                "robot0_joint_vel",
            )
            if joint_velocities is None:
                raise ValueError(
                    "RVT2 attention targets require lowdim/robot0_joint_vel.npy."
                )

            keypoints, _ = discover_rvt2_heatmap_keypoints(
                joint_velocities,
                gripper_open_from_signal(gripper_qpos),
                atol=float(cfg.get("joint_vel_atol", 0.1)),
                stopped_buffer_len=int(cfg.get("stopped_buffer_len", 4)),
                include_gripper_changes=True,
                include_final=_strict_bool(
                    cfg.get("include_final", True),
                    "rvt2_attention_target.include_final",
                ),
                mute_initial_gripper_open=_strict_bool(
                    cfg.get("mute_initial_gripper_open", True),
                    "rvt2_attention_target.mute_initial_gripper_open",
                ),
            )
            ...
            for local_t, active_idx in enumerate(active_indices.tolist()):
                next_pos = int(np.searchsorted(keypoints, local_t, side="right"))
                if next_pos >= keypoints.shape[0]:
                    continue
                target_t = int(keypoints[next_pos])
                for view, camera_matrices in camera_matrices_by_view.items():
                    row_col = _project_world_xyz_to_row_col(
                        eef_pos[target_t],
                        camera_matrices[int(global_indices[local_t])],
                        image_size=source_size,
                    )
                    if row_col is None:
                        continue
                    targets[view]["xy_px"][int(active_idx)] = np.asarray(
                        [row_col[1], row_col[0]],
                        dtype=np.float32,
                    )
                    targets[view]["valid"][int(active_idx)] = True

        self.rvt2_attention_targets = targets
    ...
    def _load_cache_arrays(self) -> None:
        ...
        self.task_embedding_episode_all = _normalize_task_embedding_array(
            self.task_embedding_episode_all,
            name="task_embedding",
        )
        try:
            ...
            self.task_language_tokens_episode_all = _normalize_task_language_tokens_array(
                self.task_language_tokens_episode_all,
                name="task_language_tokens",
            )
        except FileNotFoundError:
    ...
    def _get_task_episode_groups(self):
        ...
        if not isinstance(source_counts, list) or len(source_counts) == 0:
            raise ValueError(
                ...
                "Rebuild the merged cache with focuspool merge-datasets."
            )
