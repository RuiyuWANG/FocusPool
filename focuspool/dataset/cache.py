...
DEFAULT_CACHE_IMAGE_SIZE = 84


def archive_path(cache_dir: str | os.PathLike) -> Path:
...
def unpacked_arrays_dir(cache_dir: str | os.PathLike) -> Path:
...
def legacy_array_path(cache_dir: str | os.PathLike, rel_path: str | os.PathLike) -> Path:
    """Return the original loose-``.npy`` array path used by legacy caches."""
    return Path(cache_dir).expanduser().resolve() / rel_path


def has_packed_arrays(cache_dir: str | os.PathLike) -> bool:
    return archive_path(cache_dir).exists()


def has_legacy_arrays(cache_dir: str | os.PathLike) -> bool:
    """Return whether cache_dir looks like the older loose-array cache layout."""
    cache_dir = Path(cache_dir).expanduser().resolve()
    legacy_roots = (
        cache_dir / "lowdim",
        cache_dir / "action",
        cache_dir / "oracle",
        cache_dir / "task_instructions.npy",
    )
    return any(path.exists() for path in legacy_roots)


def _unpacked_array_path(cache_dir: str | os.PathLike, key: str) -> Path:
    return unpacked_arrays_dir(cache_dir) / f"{key}.npy"


def _unpack_is_current(cache_dir: str | os.PathLike) -> bool:
...
def ensure_unpacked_arrays(cache_dir: str | os.PathLike) -> Path:
...
def is_cache_dir(path: str | os.PathLike) -> bool:
    ...
    return (
        ...
        and (has_packed_arrays(path) or has_legacy_arrays(path))
...
def default_cache_dir_for_dataset(
    ...
    image_size: int | None = DEFAULT_CACHE_IMAGE_SIZE,
) -> Path:
    ...
    size_suffix = f"_x{int(image_size)}" if image_size is not None else ""
    return task_dir / f"{task_name}_lmdb"


def resolve_cache_dir(dataset_path: str, cache_dir: str | None = None) -> Path:
    ...
    if cache_dir is not None:
        ...
        resolved = default_cache_dir_for_dataset(path / f"{path.name}.hdf5")
    else:
    ...
    if not is_cache_dir(resolved):
        raise FileNotFoundError(
            ...
            f"{ARRAYS_ARCHIVE_NAME} or legacy .npy arrays, and build_done.flag: "
            f"{resolved}"
        )
    return resolved


def get_obs_keys(shape_meta: Dict) -> Tuple[List[str], List[str]]:
    """Split observation keys from ``shape_meta`` into RGB and lowdim groups."""
...
def load_numpy_array(cache_dir: str, rel_path: str) -> np.ndarray:
    ...
    if packed_path.exists():
        ...
        path = legacy_array_path(cache_dir, rel_path)
        if not path.exists():
            raise FileNotFoundError(
                "Missing array in cache.\n"
                f"  requested: {rel_path}\n"
                f"  packed   : {packed_path}\n"
                f"  legacy   : {path}"
            )
    try:
...
def load_task_instructions(cache_dir: str) -> List[str] | None:
...
def check_cache(cache_dir: str, dataset_path: str) -> None:
    ...
    if not os.path.exists(cache_dir):
        raise FileNotFoundError(
            ...
            "  focuspool rerender-dataset --dataset <dataset.hdf5>\n"
        )

    missing = [
        os.path.join(cache_dir, name)
        for name in ("images.lmdb", "meta.json", "build_done.flag")
        if not os.path.exists(os.path.join(cache_dir, name))
    ]
    if not has_packed_arrays(cache_dir) and not has_legacy_arrays(cache_dir):
        missing.append(os.path.join(cache_dir, ARRAYS_ARCHIVE_NAME))
    if not missing:
        return
    ...
    raise FileNotFoundError(
        ...
        + "\n\nBuild the cache first, or provide a cache in either the current "
        "packed format or the older loose-.npy format, e.g.:\n"
        "  focuspool rerender-dataset --dataset <dataset.hdf5>\n"
    )
